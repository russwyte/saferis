...
trait Codec[A: ClassTag] extends Encoder[A], Decoder[A]:
  self =>
  ...
  def transform[B: ClassTag](map: A => Task[B])(contramap: B => Task[A]): Codec[B] =
    new Codec[B]:
...
object Codec:
  def apply[A: Encoder as encoder: ClassTag: Decoder as decoder]: Codec[A] = codec[A]
  ...
  given codec[A: Encoder as enc: ClassTag: Decoder as dec]: Codec[A] with
