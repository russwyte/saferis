  ...
  val spec =
    suiteAll("interpolator"):
      test("simple interpolation"):
        val name = "Bob"
        val sql  = sql"select * from test_table where name = $name"
        assertTrue(sql.sql == "select * from test_table where name = ?")
      test("multiple interpolations"):
        val name = "Bob"
        val age  = 42
        val sql  = sql"select * from test_table where name = $name and age = $age"
        assertTrue(sql.sql == "select * from test_table where name = ? and age = ?") && assertTrue(sql.writes.size == 2)
