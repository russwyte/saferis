...
```
// Scala 3.3.8
SELECT * FROM tasks WHERE title LIKE 'Learn%' AND done = false
```
...
```
// Scala 3.3.8
insert into builder_users (name, email, age) values (?, ?, ?)
```
...
```
// Scala 3.3.8
insert into builder_users (name, email, age) values (?, ?, ?) returning *
```
...
```
// Scala 3.3.8
update builder_users set name = ?, age = ? where builder_users.id = ?
```
...
```
// Scala 3.3.8
update builder_users set email = ? where builder_users.name = ? and builder_users.age > ?
```
...
```
// Scala 3.3.8
update builder_users set age = ? where builder_users.id = ? returning *
```
...
```
// Scala 3.3.8
delete from builder_users where builder_users.id = ?
```
...
```
// Scala 3.3.8
delete from builder_users where builder_users.age < ? and builder_users.name <> ?
```
...
```
// Scala 3.3.8
delete from builder_users where builder_users.email = ? returning *
```
...
```
// Scala 3.3.8
update builder_users set age = ? where name LIKE ?
```
...
```
// Scala 3.3.8
update claim_tasks set claimedBy = ? where claim_tasks.deadline <= ? and (claim_tasks.claimedBy is null or claim_tasks.claimedUntil < ?)
```
...
```
// Scala 3.3.8
delete from claim_tasks where claim_tasks.deadline < ? and (claim_tasks.claimedBy is not null or claim_tasks.claimedUntil < ?)
```
...
```
// Scala 3.3.8
update lock_rows set expiresAt = ? where lock_rows.instanceId = ? and lock_rows.nodeId = ? returning *
```
...
```
// Scala 3.3.8
delete from lock_rows where lock_rows.instanceId = ? returning *
```
